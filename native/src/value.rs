...
use crate::js::{ToJs, ToJsMulti, FromJs};
use neon::handle::{Handle};
use neon::prelude::{Context as JsContext};
use neon::types::{JsValue, JsNull, JsUndefined, JsNumber, JsString, JsBoolean, JsArray, JsObject};
...
impl ToJsMulti for Value {
    fn to_js_multi<'a, CX: JsContext<'a>>(
        self,
        cx: &mut CX,
    ) -> Vec<Handle<'a, JsValue>> {
        match self {
            Value::Null => vec![cx.null().upcast()],
            Value::Boolean(b) => vec![cx.boolean(b).upcast()],
            Value::String(s) => vec![cx.string(s).upcast()],
            Value::Number(f) => vec![cx.number(f).upcast()],
            Value::Array(values) => {
                let r: Vec<Handle<'a, JsValue>> = values.into_iter().flat_map(|v| {
                    v.to_js(cx)
                }).collect();
                r
            }
